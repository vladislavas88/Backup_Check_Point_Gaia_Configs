...
=head1 Using the script for create backup of Check Point Gaia configs  
#===============================================================================
#
...
#
#  DESCRIPTION: Create Check Point Gaia configs backup
#
#      OPTIONS: ---
...
foreach my $cpHost (@cpAppliances) {
	...
	#my $ssh=Net::OpenSSH->new($cpHost, user=>$backupLogin, password=>$backupPassword, timeout=>30);
		...
		chomp($hostname);
		my $uname=$ssh->capture('uname -a');
		my $shConfig=$ssh->capture('clish -c "show configuration"');
		my $dynamicObjects=$ssh->capture('dynamic_objects -l');
		my $asset=$ssh->capture('clish -c "show asset all"');
		my $version=$ssh->capture('clish -c "show version all"');
		my $intMgmt=$ssh->capture('clish -c "show interface eth0" | grep mac-addr');
